...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Starnodes address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no Starnodes: URI
...
{
    // Convert starnodes:// to starnodes:
    //
    //    Cannot handle this later, because starnodes:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open starnodes.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Starnodes.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Starnodes.lnk
...
{
    return GetAutostartDir() / "starnodes.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a starnodes.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Starnodes\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the starnodes app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add starnodes app to startup item list
