...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeStarnodesAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeStarnodesAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeStarnodesAmount = strAnonymizeStarnodesAmount.remove(strAnonymizeStarnodesAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStarnodesAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeStarnodesAmount * COIN) nMaxToAnonymize = nAnonymizeStarnodesAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeStarnodesAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeStarnodesAmount));
        strAnonymizeStarnodesAmount = strAnonymizeStarnodesAmount.remove(strAnonymizeStarnodesAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStarnodesAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeStarnodesAmount == 0) {
